...
def get_parent_time_diff(g: nx.DiGraph, tweet_id, created_at) -> float:
    '''
    Returns difference in creation time of parent node and child node
    '''
    parent_id = list(g.predecessors(tweet_id))[0]
...
def get_avg_child_time_diff(g: nx.DiGraph, tweet_id, created_at):
    ...
    '''
    child_ids = list(g.successors(tweet_id))
    if len(child_ids) != 0:
        return mean([g.nodes[x]['created_at'] - created_at for x in child_ids])
    return 0
